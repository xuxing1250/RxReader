...
public class RefreshHeader extends LinearLayout implements BaseRefreshHeader {
    ...
    public boolean isRefreshing() {
        return mState < STATE_RELEASE_TO_FRESH ;
    }

    private void initView() {
        LayoutInflater.from(getContext()).inflate(R.layout.kaws_refresh_header, this);
    ...
    public void onMove(float delta ) {
        Log.d(TAG, "initView: " + getVisiableHeight());
        if (getVisiableHeight() > 0 || delta > 0) {
            setVisiableHeight((int) delta + getVisiableHeight());
            if (mState <= STATE_RELEASE_TO_FRESH) { // 未处于刷新状态，更新箭头
                if (getVisiableHeight() > mMeasuredHeight) {
                    setState(STATE_RELEASE_TO_FRESH);
                } else {
    ...
    public void setVisiableHeight(int height) {
        ...
        ViewGroup.LayoutParams lp  = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 0);
        lp.height = height;
    ...
    private void setState(int state) {
        ...
        switch (state) {
            ...
            case STATE_RELEASE_TO_FRESH:
                if (mState != STATE_RELEASE_TO_FRESH) {
                    if (!animationDrawable.isRunning()) {
